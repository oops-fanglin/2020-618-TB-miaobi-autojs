...
function mission15s(keyword){
    ...
    while(textContains(keyword).exists() ){
        ...
        textContains(keyword).findOne().click();
        sleep(3000);
        swipe(width / 2 , height - 600 , width / 2,0,5000); 
        sleep(20000);
        // if(className("android.view.View").desc(" 任务完成").exists()){
  
        // }
        // if(className("android.view.View").desc("返回618列车").exists() || textContains("返回618列车").exists() || 
        //     className("android.view.View").desc("任务完成").exists() || textContains("任务完成").exists() ||
        //     className("android.view.View").desc("继续逛逛").exists() || textContains("继续逛逛").exists() || 
        //     className("android.view.View").desc("任务已完成").exists() || textContains("任务已完成").exists()){
        //     ++successed;
