...
function justDoIt(keywords){
    var messionCount = 0;
    var currentKeyword = "";
    while(containsKeywords(keywords) != 0){
        sleep(2000);
        var currentKeywordNumber = containsKeywords(keywords);
        currentKeyword = keywords[currentKeywordNumber - 1];
        toast("检测到" + currentKeyword + "任务");
        switch(currentKeyword){
            case "去观看":
            case "签到" :
            case "去兑换" :missionClick(currentKeyword);
                break;
            case "去完成":
            case "去逛逛":
            case "去搜索":
            case "去浏览":mission15s(currentKeyword);
                break;
        }
        messionCount++;
